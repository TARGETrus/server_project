...
let OwnersPageContainer = React.createClass({
    ...
    render: function() {
        ...
        if (data.length > 0) {
            ownersTemplate = data.map(function(item, index) {
                return (
          ...
          ownersTemplate = <p>К сожалению, клиентов нет</p>
        }
        ...
        return (
            <div className="container">
                ...
                <p className='flats_count'>
                    <strong className={(data.length > 0 ? '':'none')}>Всего клиентов: {data.length}</strong>
                </p>
            </div>
        );
...
if (root_element) {
    ReactDOM.render(
        <OwnersPageContainer data={[physical_entities, legal_entities]} />,
        root_element
    );
}
